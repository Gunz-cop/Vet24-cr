...
Revisión: r4  
Estado: determinado por la auditoría independiente cuyo hash coincida; no implica implementación.
...
Esta especificación define la interfaz administrativa pendiente de I09/#35: catálogo privado, editor de fichas y horarios, mapa y evidencia, alertas temporales, reportes y propuestas permanentes mediante PR. La interfaz consume contratos propiedad de I01, I02, I04, I05, I06, I07, I08, I10 e I11; no reemplaza sus validadores, persistencia, seguridad ni operación.

Al inspeccionar la base se encontraron `src/pages/admin/index.astro` y `src/pages/admin/nueva.astro` como placeholders, sin editor funcional, y no se encontró un endpoint implementado bajo `src/pages/api/admin/**`. `src/lib/admin/auth.ts` ya expone la frontera de autenticación, método, CSRF y límite de cuerpo que deberá reutilizarse. `src/lib/schedule.ts` ya expone el motor y tipos de horario implementados localmente por I01/#27 y publicados en esta base; falta integrarlos al editor. El contenido Markdown sigue siendo la fuente permanente; D1 solo contiene estado operativo, reportes, auditoría y propuestas.

Fuera de alcance: alterar el SDD, implementar aquí código, provisionar D1/Access/GitHub, publicar fichas, fusionar PR, enviar correo o hacer altas masivas. El código de I01/#27 e I04/#30 está auditado y publicado en `main` dentro de esta base; eso describe el estado local y no prueba que D1, Access, GitHub, cron u otros servicios remotos estén provisionados.
...
I01/#27 ya implementa el contrato estricto de `schedule_v1`; esta interfaz lo consume como autoridad. I02/#28 entregará el sidecar futuro `research/clinics/<slug>.json` y su contrato de evidencia. El wire estricto omite `schedule_v1` cuando el catálogo antiguo no lo tiene; no envía `null` dentro de `clinicSchema`. La UI mantiene un `scheduleDraft: ScheduleV1|null` solo en estado local, y al modificar horario exige un `ScheduleV1` válido en el POST; si no se modifica, puede omitirlo conforme a I02.

Hay dos capas de evidencia con nombres y fronteras fijas. `EvidenceSidecar` es el objeto canónico I02 que viaja en propuestas: `{schemaVersion,slug,observedAt,fuentes,facts,conflicts}`; `status` no se persiste en él. `EvidenceView` es exclusivamente el envelope de GET: `{status:"unavailable"|"partial"|"complete",sidecar:EvidenceSidecar|null}`. `sidecar:null` solo cuando status=`unavailable`; no existe alias `sources`. Cada elemento de `fuentes` conserva 1:1 `url`, `tipo`, `consultedAt?` y `publishedAt?`, y `facts`/`conflicts` mantienen referencias, valores, fechas y resolución. El POST transmite exclusivamente `EvidenceSidecar` validado, sin `status` ni claves de envelope. Sidecar ausente es `EvidenceView.status="unavailable"`, no evidencia vacía confirmada.
...
| --- | --- | --- | --- |
| Motor de horarios | I01/#27 | `ScheduleV1`, parser, evaluación y serialización | no editar `scheduleAttr`; todos los estados se derivan del contrato central |
| Schema, skill, evidencia, UUID/slug y deuda | I02/#28 | sidecar y validador canónicos | payload completo y evidencia admitida antes de guardar/proponer |
| Auth, frontera Workers y retiro legacy | I04/#30 | identidad Access, rutas y tombstones | auth antes de datos; sin `functions/` ni vía alternativa |
| Consumidores públicos | I06/#32 | proyección estática/viva | estado y degradación coherentes en tarjeta, detalle, filtros y mapa |
| Persistencia D1, overrides, CAS y auditoría | I05/#31 | revisiones, expiración, idempotencia, rate limit y estados | escritura atómica; 409 conserva el formulario |
| Reportes D1 | I07/#33 | bandeja, resolución y retención | escritura pública veraz y contacto solo privado |
| Propuestas y runner durable | I08/#34 | outbox, estados, GitHub y retry | 202 durable; sin auto-merge |
| Esta interfaz y APIs de lectura | I09/#35 | catálogo, editor, accesibilidad y wiring | consume contratos; no reasigna propiedad |
| CI/harness e integración | I10/#36 | suites Workers/D1/E2E y gates | bundle real, fixtures reproducibles |
| Operación/preflight | I11/#37 | hosts, Access, backup, cron, alertas, rollback | despliegue deshabilitado hasta preflight |
...
El motor horario de I01/#27 y la frontera/auth de I04/#30 están implementados, auditados y publicados en `main` localmente (incluidos `src/lib/schedule.ts` y `src/lib/admin/auth.ts`); la integración completa del panel, sus APIs y la guardia 503 para configuración ausente o mutación aún no implementada siguen pendientes. Esta spec no los declara terminados ni infiere estado remoto.
...
`GET /api/admin/clinics?q=&provincia=&estado=&issue=&verification=&alerts=&reports=&cursor=&limit=` devuelve `{ items, nextCursor, baseCommit, generatedAt, counts }`, donde `counts` es exactamente `{totalFiltered:number}` calculado sobre el conjunto filtrado completo antes de paginar. `q` admite nombre, slug y zona normalizados sin tildes, máximo 100 caracteres. `limit` entero 1..100, por defecto 25; `cursor` es opaco y validado por servidor. El orden es estable por nombre normalizado y, como desempate, slug/id. Cada ítem incluye `slug`, `id`, `nombre`, provincia/zona, estado editorial, flags de verificación, problemas calculados, alerta activa, reportes pendientes, `baseCommit` y `sourceHash`.

Los parámetros son cerrados: `estado=pendiente|publicado|verificado|inactivo`; `issue=horario|datos|coordenadas|evidencia|conflicto`; `verification=pending`; `alerts=active`; `reports=pending`. Una lista usa claves repetidas (`estado=publicado&estado=verificado`), con cada valor validado, sin CSV ni JSON. Se combinan con AND entre parámetros y OR dentro de un mismo parámetro. El servidor ordena las claves repetidas por valor antes de formar cache key/cursor; el cliente no filtra una página parcial. Todos se aplican al catálogo completo antes de calcular `items`, `nextCursor` y `counts`. Ausencia de parámetro significa “sin filtro”. Si D1 no está disponible, cualquier filtro live (`alerts` o `reports`) responde 503 con `live_unavailable`, sin `items` ni `counts` live; no devuelve lista vacía ni conteo falso. El resto puede conservar campos estáticos con estado operativo no confirmado, error recuperable y mutaciones deshabilitadas.

Filtros visibles: provincia; estado editorial; problema de horario; verificación pendiente; alertas activas; reportes pendientes. Un problema lleva `code`, `field`, explicación y referencias de fuente. `unknown` o “por confirmar” deliberado se muestra como “por confirmar”, nunca como “roto”. `counts.totalFiltered` es el único contador normativo, calculado sobre el conjunto filtrado antes de aplicar `cursor`/`limit`; nunca se deriva de `items` ni se fija a números históricos. Cuando la consulta no usa filtros live y D1 está disponible, refleja el conjunto completo de datos estáticos y operativos que corresponde a esos filtros.
...
`GET /api/admin/clinics/:slug` devuelve `{clinic, evidence: EvidenceView, blobSha, baseCommit, sourceHash, overrideRevision, operationalState, proposal}`. `EvidenceView` tiene la forma descrita en §1.1; el POST usa solo su `sidecar` canónico. El slug debe existir en el catálogo publicado; 404 no revela si existe un registro fuera del catálogo. El servidor obtiene el Markdown desde la ref permitida y preserva cuerpo editorial y campos ajenos autorizados. El navegador no decide qué campos se conservan.
...
Inventario cerrado del wire de ficha (I02 es autoridad de tipos y validación):

| Campo | Tipo; requerido/nulo | UI y evidencia |
| --- | --- | --- |
| `id` | wire actual `number|string`; requerido, no nulo; normalizado a string para comparar | solo lectura en existente y se conserva; UUID generado en nueva; identidad no se deriva |
| `nombre`, `slug`, `provincia`, `zona`, `direccion` | string; requeridos, no nulos | nombre/ubicación editables; slug único y estable; evidencia para hechos publicados |
| `telefono1`, `telefono2`, `whatsapp`, `web`, `facebook`, `instagram`, `waze_url`, `maps_url` | string; opcionales, nunca nulos en wire actual (`""` = desconocido) | editables con protocolo/host; verificar cada contacto por fuente |
| `horarioTexto` | string derivado; no editable como verdad | deriva de `schedule_v1`; no evidencia por sí mismo |
| `schedule_v1` | `ScheduleV1` opcional; ausente en transición, nunca `null` en `clinicSchema` | editable estructurado; `scheduleDraft:null` solo local; requiere evidencia para marcar `schedule_verified` |
| `categoriaHorario` | enum `24/7 Emergencias\|Cierra después 21h\|Horario normal`; requerido | derivado/validado contra horario y flags; no sustituye evidencia |
| `emergencias24h`, `atiendeExoticos`, `cirugiaEmergencia` | boolean; requeridos por wire actual, no nulos | editables solo con evidencia del hecho; default falso no es “no” conocido |
| `atiendeGranja`, `atiendePeces`, `hotelMascotas` | wire actual opcional, default `false`; no nulos tras parseo actual | default es compatibilidad, no hecho; I02 definirá por separado si son requeridos para nuevas altas |
| `hotelVerificacion` | enum `confirmado\|reportado\|no-confirmado`; opcional/default | derivado de evidencia; `hotelFuente` conserva URL o referencia |
| `hotelFuente`, `verification_source`, `last_verified` | string; opcionales/default `""` | editables como referencias/fecha válida; vacío = sin dato |
| `confidence_score` | enum `high\|medium\|low`; opcional/default `medium` | derivado del sidecar, no elegido para convertir un dato en VERIFIED |
| `record_status` | enum `VERIFIED\|PARTIAL\|REVIEW_REQUIRED\|CANDIDATE_REMOVAL`; opcional/default `PARTIAL` | solo derivado por servidor desde sidecar/validación |
| `emergency_tier` | enum `Tier A\|Tier B\|Tier C\|Tier D`; opcional/default `Tier C` | derivado; Tier A exige prueba directa de I02 y calendario/capacidades de I01 |
| `phone_verified`, `address_verified`, `schedule_verified`, `emergency_verified` | boolean; opcionales/default falso | solo servidor/sidecar; falso por default no prueba falsedad |
| `has_surgery`, `has_hospitalization`, `overnight_doctor_present`, `accepts_emergency_walkins` | boolean; opcionales/default falso | editables con evidencia independiente; no inferir entre campos |
| `verification_notes` | string[]; opcional/default `[]` | conserva notas autorizadas; no sustituye `facts` ni fuentes |
| `latitude`, `longitude` | number; opcionales; null permitido en borrador futuro | editables con validación geográfica; 0/0 inválido y no sentinel; evidencia de sucursal |
| `copyDiferenciador` | string; requerido, no nulo | cuerpo editorial preservado/validado; no HTML/Markdown nuevo desde v1 |
| `estado` | enum `pendiente\|publicado\|verificado\|inactivo`; requerido | lectura/derivación editorial; no permite al cliente publicar |
| cuerpo Markdown y claves ajenas reconocidas | string/shape del documento; preservados por servidor | no se descartan al editar; schema I02 rechaza claves desconocidas nuevas |

El sidecar transportado junto con una propuesta es exactamente `{schemaVersion,slug,observedAt,fuentes,facts,conflicts}` donde `observedAt` es ISO UTC, `fuentes` es una lista de `{url,tipo,consultedAt?,publishedAt?}`, `facts` una lista de `{field,value,references,confidence}`, y `conflicts` una lista de `{field,values,references,resolution}`. Este `EvidenceSidecar` es el cuerpo canónico de `evidence` en POST; el GET lo envuelve como `EvidenceView` y añade `status` sin persistirlo en el sidecar. No existe alias `sources`. La UI exige referencias existentes, fechas válidas y slug coincidente; el servidor rechaza omisiones, tipos extra o sidecar incompleto. Snippets y redes inaccesibles son pistas/limitaciones, nunca base única de `VERIFIED`. `record_status` y flags se derivan del sidecar; `VERIFIED` y Tier A se bloquean sin evidencia admitida.

En el borrador la UI usa triestado interno `unknown|true|false`; `unknownFields` es exclusivamente estado local de `DraftState`, no se serializa ni se incluye en `clinic`, `evidence` o POST. Para nueva ficha publicable, cualquier booleano requerido en `unknown` bloquea el submit y el servidor devuelve 422 `UNKNOWN_FIELD` al validar el payload canónico. Para edición existente, la UI conserva el valor booleano histórico sin editarlo; el servidor relee la base validada y lo preserva, mientras la ausencia de `facts` afirmativos mantiene ese valor como no confirmado. No se añade `unknownFields` al schema, sidecar o registro. El cliente no agrega claves desconocidas: el servidor rechaza claves extra, mientras que campos reconocidos existentes que la UI no edita se preservan según I08. `bodyMarkdown` es el cuerpo fuera del frontmatter y se conserva en servidor.

Ejemplos esquemáticos (los tipos referenciados son los definidos en esta sección; no son JSON incompleto para enviar):

```ts
// GET existente: envelope de detalle; ClinicDTO es el inventario completo de §5.
type ExistingDetail = {
  clinic: ClinicDTO; // id histórico number|string, schedule_v1 opcional
  evidence: { status: 'complete'|'partial'|'unavailable'; sidecar: EvidenceSidecar|null };
  blobSha: string; baseCommit: string; sourceHash: string;
  overrideRevision: number; operationalState: OperationalState; proposal: ProposalView|null;
};

// POST existente: evidence es solo EvidenceSidecar, sin status/envelope/unknownFields.
type ExistingProposal = {
  slug: string; newClinic: false; baseCommit: string; blobSha: string;
  clinic: ClinicDTO; evidence: EvidenceSidecar;
};

// POST nueva: base real de main, blob inexistente explícito; ClinicDTO futuro validado por I02.
type NewProposal = {
  slug: string; newClinic: true; baseCommit: string; blobSha: null;
  clinic: NewClinicDTO; evidence: EvidenceSidecar;
};
```

`EvidenceSidecar` siempre contiene las seis claves canónicas. En los ejemplos, `ClinicDTO` significa el wire completo del inventario de §5, `NewClinicDTO` su variante futura validada por I02, `OperationalState` la proyección viva con `liveState`/`reason`, y `ProposalView` la forma de §8.3. Son alias documentales para mostrar envelopes y no payloads abreviados autorizados; el servidor valida todos los campos y rechaza omisiones, `null` inválido o `unknownFields`.
...
La interfaz separa visualmente “Guardar alerta temporal” y “Proponer cambio permanente”. La segunda muestra una forma única: `status` es exactamente `pending|branch_created|committed|pr_open|failed`; `retryable` es booleano y `errorCode` es obligatorio solo en `failed`. `stage` es opcional y, si está presente, repite el valor de `status` para compatibilidad, nunca introduce otro vocabulario. `merged` y `deployed` son observaciones separadas, solo tras consultar GitHub y `build-info`; no son etapas que la UI pueda declarar por éxito del POST. `pr_open` sigue mostrando “PR creado, pendiente de revisión”. Un 409 de base conserva campos, marca los cambios conflictivos y ofrece recargar/reaplicar. Un error GitHub o 429 no borra el borrador ni crea otra propuesta al reintentar con la misma clave. El outbox y el runner son I08; la integración y harness son I10/I11. Esta UI no usa `waitUntil` como almacenamiento.

Una nueva ficha permanece como borrador local mientras falten datos. Para proponerla como publicable, el servidor exige siempre latitud y longitud válidas, no `0,0`, evidencia verificable de ubicación/sucursal, identidad, slug, campos requeridos, evidencia y cuerpo. Coordenadas `null`, fuente inaccesible o limitación explícita mantienen el borrador local y producen 422; no se incluyen en POST ni crean operación/PR publicable. No se crea ruta pública antes de merge y build. Cancelar un borrador con cambios requiere confirmación; el servidor nunca elimina una ficha publicada por esta acción.

### 8.3 Operaciones

`GET /api/admin/operations/:id` devuelve únicamente la operación autorizada: `{operationId,status,stage,attempts,prUrl,errorCode,retryable,updatedAt}`. `status` es el estado durable único de I08 (`pending|branch_created|committed|pr_open|failed`); `stage`, si se entrega, debe ser igual a `status`. En `failed`, `retryable` y `errorCode` son obligatorios. `merged`/`deployed` solo son observaciones verificadas. No expone payload completo ni secretos. Reintento de una operación fallida usa `POST /api/admin/operations/:id/retry` con Idempotency-Key y motivo, conforme a I08; la UI solo habilita esa acción para error recuperable y vuelve a validar auth, allowlist, base y auditoría.
...
En pérdida de red se conserva el formulario en memoria y se reenvía **la misma petición con la misma `Idempotency-Key`**; no se genera un endpoint de consulta por clave ni una clave nueva. El servidor devuelve el resultado original o `REQUEST_IN_PROGRESS`, y la UI solo habilita una nueva intención después de resolver ese estado. En D1 caído, la proyección que sí pueda renderizarse conserva campos estáticos, pero `operationalState` y `override` llevan `liveState:"UNCONFIRMED"`, `reason:"live_unavailable"`; nunca se devuelve `override:null` como si la lectura hubiera sido exitosa. Los filtros `alerts=active` y `reports=pending` responden 503 sin `items`/`counts` live; catálogo y detalle muestran error/reintento y deshabilitan mutaciones. Contacto y horario base permanecen visibles. En Access ausente o inválido se falla cerrado.
...
| --- | --- |
| I09-A1 / R04 | E2E busca sin tildes, combina filtros, pagina con cursor, verifica `counts:{totalFiltered:number}` antes de paginar y distingue unknown de roto; filtros live con D1 caído reciben 503 sin counts live |
...
1. I04/#30 integra la frontera ya implementada, rutas canónicas, auth y rechazo de legacy; comprobar que `/functions` no ofrece escritura, que auth se ejecuta antes de assets y completar el gate 503 tanto para configuración ausente como para mutaciones aún no implementadas.
2. I01/#27 aporta el motor ya implementado; I02/#28 entrega schema, skill, validador, sidecar de evidencia, deuda y fixtures futuros; fijar la misma dependencia YAML para runtime y CI.
3. I05/#31 aplica migraciones aditivas, tablas de overrides/auditoría/idempotencia, CAS, expiración, rate limit y ledger; validar DB vacía y baseline existente.
...
8. I08/#34 conecta propuestas/runner y I09 conecta operaciones; probar 202, leases, estados normativos, fallo/429, reconciliación y PR sin auto-merge.
9. I07/#33 integra reportes; I06/#32 verifica proyección pública; I10/#36 ejecuta suites unitarias, contratos, integración Workers/D1, E2E, accesibilidad y regresión; corregir hasta cumplir la tabla anterior.
10. I11/#37 ejecuta preflight de hosts, Access, bindings, ledger, secretos, GitHub App, cron, backup y rollback en staging. La habilitación remota queda fuera de esta entrega y requiere su proceso autorizado.

## 13. Criterio de promoción de esta spec

La promoción de esta spec queda determinada por la auditoría independiente cuyo SHA256 coincida con el archivo exacto y cuyo veredicto sea PASS. Un auditor puede exigir correcciones; este documento no se autoaprueba. La existencia de esta spec tampoco prueba que el panel, D1, Access, GitHub, Cron o producción estén provisionados.
