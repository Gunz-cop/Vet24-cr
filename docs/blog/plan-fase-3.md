...
Última revisión: **2026-09-08** (correcciones: enlace de navegación; frontera de artefactos y borradores; línea base del escaneo; evidencia versionada; autoría de organización y ausencia de revisor veterinario; geometría de bloques con elemento añadido).
...
- `docs/blog/autoria.md` registra: el nombre público de la firma; el tipo de entidad (`Organization` o `Person`) que debe emitir el JSON-LD; la aprobación explícita del titular del sitio; y los artículos y commits a los que aplica.
- **El registro no lleva nombres personales.** Este repositorio es público: un nombre escrito aquí queda visible de forma permanente, y en el historial de git aunque después se borre. Con una firma de organización no existe ningún nodo `Person` que rellenar, así que ningún nombre personal es necesario para que la atribución sea veraz. La responsabilidad editorial recae en el titular del sitio, identificable por la titularidad del dominio y de la cuenta del repositorio; no hace falta escribirla. No se versionan nombres personales, documentos de identidad ni direcciones de correo. Si en el futuro se quisiera una firma personal —la señal E-E-A-T más fuerte para este contenido—, es una decisión del titular, no un requisito que ninguna sesión pueda imponer.
...
- La entrada de navegación al blog está implementada y condicionada: con cero artículos publicados no se renderiza en ninguna página, y con al menos uno enlaza a `/blog/` con su barra final, en cabecera y en pie. Se demuestra con una prueba por cada lado de la condición y con una fixture publicada; no se acepta la condición comprobada en un solo sentido. El enlace es `<a href>` normal, rastreable y enfocable por teclado. **En móvil debe ser alcanzable**: la entrada de cabecera es sólo de escritorio, así que el pie es la vía real a 390×844 y se mide con navegador, no por clases CSS.

  **Qué se le exige a la geometría aquí, y qué no.** La tolerancia de ≤1 CSS px de §5/B2 protege los controles de emergencia de ficha y zona, donde **nada debe moverse**; ahí sigue firme y sin excepciones. No aplica a una fila donde añadimos un elemento a propósito: en un contenedor flex, insertar un enlace desplaza a sus hermanos por construcción, de modo que exigir ambas cosas es pedir un imposible. Medido en la ficha de HEMS a 1440×900, la entrada de pie desplaza los seis enlaces previos −52,03 px; la verificación independiente de B3 midió además −122,83 px y −63,11 px en otras combinaciones. Ninguna de esas cifras es un defecto.

  El criterio correcto para un bloque al que se le añade un elemento es: **ningún destino previo desaparece ni cambia de `href`**; **el orden relativo de foco de los elementos previos se conserva**; **ninguno queda fuera del viewport ni cubierto** por el elemento nuevo; y en móvil no se pierde nada que antes fuera alcanzable. El desplazamiento en sí se registra con su medida y no bloquea.
