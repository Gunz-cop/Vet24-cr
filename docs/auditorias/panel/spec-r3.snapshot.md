...
Los parámetros son cerrados: `estado=pendiente|publicado|verificado|inactivo`; `issue=horario|datos|coordenadas|evidencia|conflicto`; `verification=pending`; `alerts=active`; `reports=pending`. Se combinan con AND entre parámetros y OR dentro de una lista del mismo parámetro. Todos se aplican en servidor al catálogo completo antes de calcular `items`, `nextCursor` y `counts`; el cliente no filtra una página parcial. Ausencia de parámetro significa “sin filtro”. Si D1 no está disponible, `alerts`/`reports` no se inventan: la respuesta lleva `degraded:{liveState:"UNCONFIRMED",reason:"live_unavailable",fields:["alerts","reports"]}`, error recuperable y mutaciones deshabilitadas.
...
En pérdida de red se conserva el formulario en memoria y se reenvía **la misma petición con la misma `Idempotency-Key`**; no se genera un endpoint de consulta por clave ni una clave nueva. El servidor devuelve el resultado original o `REQUEST_IN_PROGRESS`, y la UI solo habilita una nueva intención después de resolver ese estado. En D1 caído, el catálogo base y todos sus campos estáticos siguen visibles, pero los campos de alerta/reportes llevan `liveState:UNCONFIRMED`, `reason:live_unavailable`; la respuesta no usa `override:null` como si la lectura hubiese sido exitosa, muestra error/reintento y deshabilita mutaciones. Contacto y horario base permanecen visibles. En Access ausente o inválido se falla cerrado.
