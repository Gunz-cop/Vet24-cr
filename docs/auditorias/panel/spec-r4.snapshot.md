...
Revisión: r4  
Estado: **CANDIDATA**. Determinado por la auditoría independiente cuyo hash coincida; no implica implementación.
...
I01/#27 ya implementa el contrato estricto de `schedule_v1`; esta interfaz lo consume como autoridad. I02/#28 entregará el sidecar futuro `research/clinics/<slug>.json` y su contrato de evidencia. Durante la transición, el endpoint de detalle puede devolver campos actuales y `schedule_v1: null`; la UI ofrece edición estructurada solo cuando el servidor acepta el contrato nuevo y nunca reconstruye evidencia desde un default. El DTO único de evidencia usa `fuentes` (no `sources`) en lectura y escritura: `evidence: {status:"unavailable"|"partial"|"complete", facts, conflicts, fuentes}`. Cada elemento de `fuentes` conserva 1:1 `url`, `tipo`, `consultedAt?` y `publishedAt?`; sus identificadores y fechas no se renombran ni pierden al construir el sidecar I02. Sidecar ausente es `unavailable`, no evidencia vacía confirmada.
...
| --- | --- | --- |
| `id` | string normalizado; requerido, no nulo | solo lectura en existente; UUID generado en nueva; identidad no se deriva; evidencia de identidad |
...
| `schedule_v1` | `ScheduleV1` opcional/null en transición | editable estructurado; requiere evidencia para marcar `schedule_verified` |
...
| `emergencias24h`, `atiendeExoticos`, `cirugiaEmergencia`, `atiendeGranja`, `atiendePeces`, `hotelMascotas` | boolean; requeridos por schema, nunca nulos | editables solo con evidencia del hecho; default falso no es “no” conocido |
...
El sidecar transportado junto con una propuesta es exactamente `{schemaVersion,slug,observedAt,fuentes,facts,conflicts}` donde `observedAt` es ISO UTC, `fuentes` es una lista de `{url,tipo,consultedAt?,publishedAt?}`, `facts` una lista de `{field,value,references,confidence}`, y `conflicts` una lista de `{field,values,references,resolution}`. Este es el mismo DTO de `evidence` usado por GET y POST; no existe alias `sources`. La UI exige referencias existentes, fechas válidas y slug coincidente; el servidor rechaza omisiones, tipos extra o sidecar incompleto. Snippets y redes inaccesibles son pistas/limitaciones, nunca base única de `VERIFIED`. `record_status` y flags se derivan del sidecar; `VERIFIED` y Tier A se bloquean sin evidencia admitida.

En el borrador la UI usa triestado interno `unknown|true|false`; `unknown` se serializa como una lista explícita `unknownFields` del DTO de borrador y nunca como `false`, `null` ni omisión ambigua en una ficha publicable. Para nueva ficha publicable, cualquier booleano requerido en `unknown` produce 422 `UNKNOWN_FIELD` y bloquea el POST. Para edición existente, el servidor conserva el booleano histórico sin cambiarlo y añade el campo a `unknownFields`/estado de evidencia; no lo trata como hecho nuevo ni permite usarlo para marcar VERIFIED. El cliente no agrega claves desconocidas: el servidor rechaza claves extra, mientras que campos reconocidos existentes que la UI no edita se preservan según I08. `bodyMarkdown` es el cuerpo fuera del frontmatter y se conserva en servidor.
