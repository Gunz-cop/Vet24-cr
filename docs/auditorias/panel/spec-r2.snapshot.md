...
El motor horario de I01/#27 y la frontera/auth de I04/#30 ya tienen alcance local en la base (incluidos `src/lib/schedule.ts` y `src/lib/admin/auth.ts`); la integración completa del panel, sus API y su gate 503 de configuración siguen pendientes. Esta spec no los declara terminados.
...
1. I04/#30 prepara frontera, rutas canónicas, auth y rechazo de legacy; comprobar que `/functions` no ofrece escritura, que auth se ejecuta antes de assets y que el gate 503 de configuración funciona.
2. I01/#27 e I02/#28 entregan motor, schema, skill, validador, evidencia, deuda y fixtures; fijar la misma dependencia YAML para runtime y CI.
